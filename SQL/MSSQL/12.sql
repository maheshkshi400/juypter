...
SELECT TOP 1 SAL FROM(
    SELECT DISTINCT TOP 2 SAL,DEPTNO
    FROM EMP5 
    ORDER BY SAL DESC
)AS RASULT
ORDER BY SAL;
...
WITH RESULT AS (
    SELECT SAL, DENSE_RANK() OVER (PARTITION BY DEPTNO ORDER BY SAL DESC) AS DENSE_RANK 
    FROM EMP5
)
...
SELECT MAX(ROWID) FROM EMP5
UNION
SELECT MIN(ROWID) FROM EMPS;
