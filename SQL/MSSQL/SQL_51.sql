...
--Email activity rank is defined by the total number of emails sent. The User with the highest number 
--of emails sent will have a rank of 1 and so on. Output the user, total emails and their activity rank. 
...
WITH CTE_ACTIVITYRANK AS(
    SELECT From_User,COUNT(1) AS TOTAL_EMAILS FROM google_gmail_emails
    GROUP BY From_User
)
SELECT From_User,TOTAL_EMAILS,
ROW_NUMBER() OVER (ORDER BY TOTAL_EMAILS DESC,From_User) AS RANK
FROM CTE_ACTIVITYRANK