...
WHERE SAL IN (SELECT MAX(sal) FROM EMPLOYEES.dbo.EMP2 GROUP BY deptno);

--by using dense rank

SELECT * FROM (SELECT *, DENSE_RANK() OVER (PARTITION BY deptno ORDER BY sal DESC) AS RNK 
FROM EMPLOYEES.dbo.EMP2) AS RESULT WHERE RNK=1;
...
WITH HIGH_RANK AS
(
SELECT *, DENSE_RANK() OVER (ORDER BY SAL DESC) AS RNK FROM EMPLOYEES.dbo.EMP2
)
