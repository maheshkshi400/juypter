...
SELECT * FROM train_arrivals1

SELECT * FROM train_departures

CREATE TABLE train_arrivals1 (
 train_id INT,
...
WITH train_schedule AS (
  SELECT arrival_time AS event_time, 1 AS event
  FROM train_arrivals
  UNION ALL
  SELECT departure_time AS event_time, -1 AS event
  FROM train_departures
),
running_total AS (
  SELECT event_time,
         SUM(event) OVER (ORDER BY event_time) AS total_events
  FROM train_schedule
)
SELECT MAX(total_events) AS min_platform_required
FROM running_total;